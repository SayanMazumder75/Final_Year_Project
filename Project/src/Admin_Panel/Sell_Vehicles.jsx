...
export default function Dashboard() {
  ...
  // Dummy Data (replace with DB in future)
  const soldCars = [
    {
      id: 1,
      name: "Tesla Model 3",
          ...
          <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6">
            ...
            <div className=" flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-5 gap-3">
              ...
              <input
                ...
                className="w-full sm:w-64 px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>

            {/* Table */}
            <div className="overflow-x-auto">
              <table className="min-w-full text-xs sm:text-sm text-gray-700 border border-gray-200 rounded-lg">
                ...
                <tbody>
                  {filteredCars.length > 0 ? (
                    filteredCars.map((car, index) => (
                      ...
                      >
                        <td className="py-2 px-3 sm:py-3 sm:px-4 font-medium">{car.name}</td>

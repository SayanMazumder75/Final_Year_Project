...
export default function UsersPage() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await fetch("http://localhost:5000/user/users"); // Fetch all users
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/user/users/${userId}`, {
        method: "DELETE",
      });
          ...
          {users.map((user) => (
            ...
            >
              ...
              <div className="flex flex-col items-center">
                ...
                />
                <h2 className="text-lg sm:text-xl font-semibold mt-3">{user.name}</h2>
                <p className="text-gray-500 text-xs sm:text-sm text-center">{user.email}</p>
              </div>
